...
from .models import Book, Review#, BucketList
...
def books(request):
    """Show all books"""
    books = Book.objects.filter(owner=request.user, in_bucket_list=False).order_by('date_added')
    context = {'books': books}
...
@login_required
def books_in_bucket_list(request):
    """Show all books"""
    books_in_list = Book.objects.filter(owner=request.user, in_bucket_list=True).order_by('date_added')
    context = {'books_in_list': books_in_list}
    return render(request, 'reading_log/bucket_list.html', context)


@login_required
def bucket_list_item(request, book_id):
    """Show a single book and all of its reviews"""
    bucket_list_item = Book.objects.get(id=book_id)

    # Make sure the topic belongs to the current user
    if bucket_list_item.owner != request.user:
        raise Http404

    reviews = bucket_list_item.review_set.order_by('-date_added')
    context = {'bucket_list_item': bucket_list_item, 'reviews': reviews}
    return render(request, 'reading_log/bucket_list_item.html', context)


@login_required
def new_bucket_list_item(request):
    """Create and add a new book"""
    if request.method != 'POST':
        # No data was submitted, create a blank form
        form = BookForm()
        ...
        if form.is_valid():
            new_book = form.save(commit=False)
            new_book.owner = request.user
            new_book.in_bucket_list = True
            new_book.save()
            form.save()
            return redirect('reading_log:bucket_list')

    # Display a blank or invalid form
    context = {'form': form}

    return render(request, 'reading_log/new_bucket_list_item.html', context)
